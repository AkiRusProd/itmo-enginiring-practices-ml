"""
Модуль для оценки качества лучшей модели.

Скрипт загружает сохраненную модель-чемпиона, проводит валидацию на отложенной
выборке, вычисляет метрики (Accuracy, Precision, Recall, F1), строит
матрицу ошибок (Confusion Matrix) и логирует результаты в ClearML и локальные файлы.
"""
...
def evaluate_model():
    ...
    2. Загружает и разбивает данные.
    3. Вычисляет метрики классификации.
    4. Строит матрицу ошибок и график важности признаков.
    5. Отправляет отчеты в ClearML и сохраняет JSON с метриками локально.

    Raises:
        FileNotFoundError: Если модель или файл с данными не найдены.
    """
